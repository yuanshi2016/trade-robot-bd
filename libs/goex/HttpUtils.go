...
func NewHttpRequestWithFasthttp(client *http.Client, reqMethod, reqUrl, postData string, headers map[string]string) ([]byte, error) {
	...
	if resp.StatusCode() != 200 {
		...
		return nil, errors.New(fmt.Sprintf(`{"code":%d, "desc":%s}`, resp.StatusCode(), string(resp.Body())))
	}
...
func NewHttpRequest(client *http.Client, reqType string, reqUrl string, postData string, requstHeaders map[string]string) ([]byte, error) {
	...
	if resp.StatusCode != 200 {
		return nil, errors.New(fmt.Sprintf(`{"code":%d, "desc":%s}`, resp.StatusCode, string(bodyData)))
	}
...
func HttpPutData(client *http.Client, reqUrl string, postData string, headers map[string]string) ([]byte, error) {
	if headers == nil {
		headers = map[string]string{}
	}
	headers["Content-Type"] = "application/x-www-form-urlencoded"
