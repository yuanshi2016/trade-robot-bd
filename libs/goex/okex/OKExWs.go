...
func (okV3Ws *OKExV3Ws) handle(msg []byte) error {
	logger.Infof("receive call back msg %s", string(msg))
	if string(msg) == "pong" {
		return nil
	}
	...
	if err != nil {
		logger.Error(err)
		err = okV3Ws.respHandle("error", []byte(err.Error()))
		return err
	}

	if wsResp.ErrorCode != nil {
		logger.Error(string(msg))
		err = okV3Ws.respHandle("error", msg)
	...
	if wsResp.Table != "" {
		...
		if err != nil {
			logger.Error("biz ws data error:", err)
		}
		return err
	}
