	...
	go func() {
		...
		for {
			...
			case <-intervalTimer.C:
				...
				if err != nil {
					logger.Errorf("[hbdm] get contract info error=%+v", err)
					goto reset
				}
...
func (dm *Hbdm) GetKlineRecords(contract_type string, currency CurrencyPair, period KlinePeriod, size, since int) ([]FutureKline, error) {
