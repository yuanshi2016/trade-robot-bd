...
func (hbpro *HuoBiPro) GetAccount() (*Account, error) {
	...
	for _, v := range list {
		...
		case "trade":
			...
			subAccMap[currency].Balance = balance
		case "frozen":
...
func (hbpro *HuoBiPro) GetTickers() ([]*Ticker, error) {
	...
	for _, tickmap2 := range respmap["data"].([]interface{}) {
		tickmap2, ok := tickmap2.(map[string]interface{})
		if !ok {
			log.Println("huobi tickMap not ok")
			continue
		}
		ticker := &Ticker{}
		if symbol, ok := tickmap2["symbol"]; ok {
			ticker.Symbol = strings.ToUpper(symbol.(string))
		}
		ticker.Date = ToUint64(respmap["ts"])
		ticker.Open = ToFloat64(tickmap2["open"])
		ticker.Last = ToFloat64(tickmap2["close"])
		ticker.Low = ToFloat64(tickmap2["low"])
		ticker.High = ToFloat64(tickmap2["high"])
		ticker.Vol = ToFloat64(tickmap2["vol"])
...
// 倒序
func (hbpro *HuoBiPro) GetKlineRecords(currency CurrencyPair, period KlinePeriod, size, since int) ([]Kline, error) {
