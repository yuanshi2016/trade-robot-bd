...
func (e *ExOrderRepo) GetExchangeApiList(userId string) ([]*response.ExchangeApiResp, error) {
	...
	//for _, resp := range apiResp {
	//
	//	posResp, err := e.GetExchangePos(user_id, resp.ExchangeName)
	//	if err != nil {
...
func (e *ExOrderRepo) GetExchangePos(userId, ex string) ([]*pb.ExchangePos, error) {
	...
	for _, apiInfo := range apiList {
		...
		if exchangeClient == nil {
			logger.Warn("No exchange client")
			continue
		}
