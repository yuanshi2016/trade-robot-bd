...
func (e *ExOrderRepo) GetTradeSymbols(exchange, symbol string) ([]*pb.Symbol, error) {
	...
	//})
	//if errTick != nil {
	//	return nil, response.NewDataNotFound(ErrID, errTick.Error())
	//}
	//
	//var ticks []quoteCron.Ticker
	//if err := jsoniter.Unmarshal(ticksResp.Ticks, &ticks); err != nil {
	//	logger.Warnf("解析tick数据失败 GetTradeSymbols %v", err)
	//}
	//logger.Infof("获取行情: %+v 解析：%+v", string(ticksResp.Ticks), ticks)
	...
	//
	//}
	logger.Infof("获取行情:%+v", symbolList)
