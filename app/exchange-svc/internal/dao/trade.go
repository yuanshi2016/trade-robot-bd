...
func (d *Dao) TradeCount(uid, strategyID string) (count int32, err error) {
	if err = d.db.Table(TABLE_WQ_TRADE).Where("user_id=? and strategy_id=?", uid, strategyID).Count(&count).Error; err != nil {
...
func (d *Dao) GetProfitByID(uid, strategyID string) (*model.WqProfit, error) {
	...
	ret := d.db.Table(TABLE_WQ_PROFIT).Where("strategy_id=?", strategyID).First(wqProfit)
	if err := ret.Error; err != nil && !ret.RecordNotFound() {
		logger.Warnf("GetProfitByID has err %v id:%v", err, strategyID)
		return nil, err
	}
	return wqProfit, nil
}
...
func (d *Dao) GetProfitListBySql(sql string, orderBy string) (wqProfitList []*model.WqProfit) {
	if err := d.db.Table(TABLE_WQ_PROFIT).Where(sql).Order(orderBy).Find(&wqProfitList).Error; err != nil {
