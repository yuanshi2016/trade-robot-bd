...
func changeCount(grid *grid.Grid) {
	...
	if len(splitArr) == 2 {
		...
		grid.SellQuantity, _ = decimal.NewFromFloat(grid.SellQuantity).Round(subLen).Float64()
		grid.BuyQuantity, _ = decimal.NewFromFloat(grid.BuyQuantity).Round(subLen).Float64()
	}
	priceArr := strings.Split(fmt.Sprint(grid.Price), ".")
	if len(priceArr) == 2 {
		var roundLen = len(priceArr[1])
		var subLen = int32(roundLen - 1)
		grid.Price, _ = decimal.NewFromFloat(grid.Price).Round(subLen).Float64()
	}
}
