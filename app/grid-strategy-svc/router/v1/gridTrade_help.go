...
func (g *gridTradeForm) placeLimitOrder(grids []*grid.Grid, handleKeys map[int]bool) ([]*model.TradeRecord, error) {
	records := make([]*model.TradeRecord, 0)
	...
	for k, gd := range grids {
		...
		if k > g.GridBasisNO {
			...
			amount = fmt.Sprintf("%v", gd.BuyQuantity)
			quantity = gd.BuyQuantity
			volume = gd.Price * gd.BuyQuantity
		} else {
			// 卖出限价单
			side = "sell"
			clientOrderID = model.NewGridClientOrderID(g.Exchange, side, g.gridStrategyID, k)
			amount = fmt.Sprintf("%v", gd.SellQuantity)
			quantity = gd.SellQuantity
			volume = gd.Price * gd.SellQuantity
		}

		//price = fmt.Sprintf("%v", grid.Price)
		price = strconv.FormatFloat(gd.Price, 'f', -1, 64)
		logger.Warnf("下单数量: %+v", amount)
...
func (c *cancelGridStrategyForm) closePosition() error {
	...
	logger.Infof("开始执行ID：%+v  平仓：%+v", c, needClosePosition)
	// 平仓
...
func getStatisticalInfo(uid string, strategyID string) (*statisticalInfoResp, error) {
	...
	//}
	//eva, err := client.NewExOrderClient(env.EtcdAddr).GetUserStrategyEva(context.Background(), req)
	//if err != nil {
	//	return nil, err
	//}
	//ek.Code = 0
	//ek.Data.RateReturn = eva.RateReturn
	//ek.Data.RealizeProfit = eva.RealizeProfit
	//ek.Data.RateReturnYear = eva.RateReturnYear
	//ek.Data.EvaDailyList = make([]EvaDaily, 0)
	//if len(eva.EvaDailyList) != 0 {
	//	for _, daily := range eva.EvaDailyList {
	//		var d = EvaDaily{
	//			Date:        daily.Date,
	//			ProfitDaily: daily.ProfitDaily,
	//		}
	//		ek.Data.EvaDailyList = append(ek.Data.EvaDailyList, d)
	//	}
	//}
	//
